...
public class WinNumDAO {
    private static HashMap<Integer, WinNumDB> winningNumberHash = new HashMap<>();  //회차:[번호]

    //서버에서 로또 번호를 불러들이면 내부저장소에 저장을 하고 내부저장소에 것을 퍼플릭으로 읽어 들여져있는건 보여준다.
    public static WinNumDB getWinNumSet(Context context, int drwNo){
    ...
    public static boolean isDeviceFileData(Context context, int drwNo){
        WinNumDB winNumSet = (WinNumDB) (DeviceFile.getData(
                context,
    ...
    private static WinNumDB getWinNumSetFromServer(Context context, int drwNo) {
        ...
        StringRequest request = new StringRequest(Request.Method.GET, url,
                new Response.Listener<String>() {
                    ...
                    public void onResponse(String response) {
                        ...
                        WinNumDB winNumSet = new WinNumDB(numbers, bNum);
                        winningNumberHash.put(drwNo, winNumSet);
                    }
                },
